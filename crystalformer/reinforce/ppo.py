...
def train(key, optimizer, opt_state, loss_fn, logp_fn, batch_reward_fn, ppo_loss_fn, sample_crystal, composition, params, epoch_finished, epochs, ppo_epochs, batchsize, path):
    ...
    for epoch in range(epoch_finished+1, epoch_finished+epochs+1):
        ...
        # Since rewards = -energy, higher rewards = lower energy
        advantages = jnp.zeros((batchsize,))
        num_matched = jnp.sum(formula_match)
        k = jnp.maximum(1, jnp.int32(jnp.ceil(0.1 * num_matched)))
        # Use top_k to find the k highest rewards (lowest energies) among matched formulas
        # Pad rewards_matched to ensure we have enough elements for top_k
        k_safe = jnp.minimum(k, num_matched)
        top_k_rewards = jax.lax.top_k(rewards_matched, k_safe)[0]
        threshold = jnp.min(top_k_rewards)
